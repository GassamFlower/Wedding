<template>
  <div>
    ...
    <el-dialog v-model="showForm" :title="editForm._id?'编辑道具':'新建道具'" width="500px">
      ...
      <template #footer><el-button @click="showForm=false">取消</el-button><el-button type="primary" style="background:#c19a50;border-color:#c19a50" :loading="saving" @click="saveProp">{{ saving?'保存中...':'保存' }}</el-button></template>
    </el-dialog>
  </div>
</template>
<script setup>
...
<style scoped>
/* ═══════════════════════════════════════════════
   ENHANCED PROPS — Design Canvas Migration
   从设计画布迁移的管理端道具页增强样式
   ═══════════════════════════════════════════════ */
